...
async function loadCSV(season,div){
  ...
  try{
    for(let attempt=0;attempt<2;attempt++){
      try{
        const r=await fetchT(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.19.2","Accept":"text/csv,text/plain,*/*"}});
...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetchT(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.19.2","Accept":"text/csv,text/plain,*/*"}
    });
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetchT(url,{headers:{"User-Agent":"MatchEdge/7.19.2"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetchT(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function structuredLeagueFixturesForDate(code,date){
  ...
  if(!l)return[];
  const k=`structured-v7192:${code}:${date}`,c=getCache(k);if(c)return c;
  let out=[];
  if(slug)try{
    const compact=date.replaceAll("-","");
    const url=`https://site.api.espn.com/apis/site/v2/sports/soccer/${slug}/scoreboard?dates=${compact}`;
...
async function fixturesForThreeDays(centerDate){
  const k=`daily-v7192:${centerDate}`,cached=getCache(k); if(cached) return cached;
  const dates=[centerDate];

  // IMPORTANT: fixture rendering must never wait indefinitely for country-specific
  // recovery sources. Give the recovery group a strict budget and continue with
  // the generic daily feeds if one provider is slow.
  ...
  const directBudget=await Promise.race([
  ...
  // Do not wait for every public provider. Some federation/public endpoints can
  // take tens of seconds even when individual fetches have timeouts.
  const fallbackBudget=await Promise.race([
    fallbackSettledPromise.then(rows=>({done:true,rows})),
    new Promise(resolve=>setTimeout(()=>resolve({done:false,rows:[]}),9000))
  ]);
  const fallbackSettled=fallbackBudget.rows;
  ...
  if(!fallbackBudget.done){
    fallbackSettledPromise.then(rows=>{
      const late=[];
      for(const r of rows){
        if(r.status==="fulfilled"&&Array.isArray(r.value?.v))late.push(...r.value.v);
      }
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.19.2"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.2","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
