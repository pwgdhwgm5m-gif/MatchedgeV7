...
async function loadCSV(season,div){
  ...
  try{
    for(let attempt=0;attempt<2;attempt++){
      try{
        const r=await fetchT(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.19.3","Accept":"text/csv,text/plain,*/*"}});
...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetchT(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.19.3","Accept":"text/csv,text/plain,*/*"}
    });
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetchT(url,{headers:{"User-Agent":"MatchEdge/7.19.3"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetchT(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function structuredLeagueFixturesForDate(code,date){
  ...
  if(!l)return[];
  const k=`structured-v7193:${code}:${date}`,c=getCache(k);if(c)return c;
  let out=[];
  if(slug)try{
    const compact=date.replaceAll("-","");
    const url=`https://site.api.espn.com/apis/site/v2/sports/soccer/${slug}/scoreboard?dates=${compact}`;
...
async function fixturesForThreeDays(centerDate){
  const k=`daily-v7193:${centerDate}`,cached=getCache(k); if(cached) return cached;
  const dates=[centerDate];
  let apiRows=[],fdRows=[],espnRows=[],officialRows=[],sportsDbRows=[],romaniaRows=[];

  // Start ALL fixture sources at the same time. Previous versions waited for
  // direct recoveries before even starting the main public feeds, which made
  // the page appear frozen on Render.
  const directPromise=Promise.allSettled([
  ...
  const apiPromise=API_KEY
    ? apiFootball(`/fixtures?date=${encodeURIComponent(centerDate)}&timezone=${encodeURIComponent(APP_TIME_ZONE)}`,300000)
        .then(rows=>(rows||[]).map(mapFixture).filter(Boolean))
        .catch(e=>{console.warn("Daily API fixtures:",e.message);return[]})
    : Promise.resolve([]);

  // Dynamic id refresh is background-only; never block fixture rendering.
  ...
  // Each source family gets its own concurrent budget. Total response time is
  // therefore the maximum budget, not the sum of all provider waits.
  const [directBudget,fallbackBudget,apiBudget]=await Promise.all([
    Promise.race([
      directPromise.then(rows=>({done:true,rows})),
      new Promise(resolve=>setTimeout(()=>resolve({done:false,rows:[]}),4500))
    ]),
    Promise.race([
      fallbackSettledPromise.then(rows=>({done:true,rows})),
      new Promise(resolve=>setTimeout(()=>resolve({done:false,rows:[]}),6500))
    ]),
    Promise.race([
  ...
  const directValues=directBudget.done
    ? directBudget.rows.map(r=>r.status==="fulfilled"?(r.value||[]):[])
    : [[],[],[],[],[]];
  const [denDirect,den2Direct,belDirect,rplDirect,ruscDirect]=directValues;

  for(const r of fallbackBudget.rows||[]){
  ...
  // Slow providers continue in the background. Their results are available on
  // the next refresh, but they can no longer keep "Fikstür yükleniyor..." open.
  if(!directBudget.done){
    directPromise.then(rows=>{
      const late=rows.map(r=>r.status==="fulfilled"?(r.value||[]):[]).flat();
      if(late.length)setCache(`direct-fixtures-late:${centerDate}`,late,300000);
    }).catch(()=>{});
  }
  if(!fallbackBudget.done){
    fallbackSettledPromise.then(rows=>{
      const late=[];
      for(const r of rows)if(r.status==="fulfilled"&&Array.isArray(r.value?.v))late.push(...r.value.v);
      if(late.length)setCache(`public-fixtures-late:${centerDate}`,late,300000);
    }).catch(()=>{});
  }
  if(!apiBudget.done){
    apiPromise.then(v=>{if(v?.length)setCache(`api-fixtures-late:${centerDate}`,v,300000)}).catch(()=>{});
  }else if(apiRows.length){
    setCache(`api-fixtures-late:${centerDate}`,apiRows,300000);
  }

  const lateDirect=getCache(`direct-fixtures-late:${centerDate}`)||[];
  const latePublic=getCache(`public-fixtures-late:${centerDate}`)||[];
  ...
  let merged=mergeFixtures(apiRows,[...fdRows,...espnRows,...officialRows,...sportsDbRows,...romaniaRows,...denDirect,...den2Direct,...belDirect,...rplDirect,...ruscDirect,...lateDirect,...latePublic]).filter(f=>dates.includes(f.localDate));
  merged=mergeFixtures(merged,[]);

  // Missing country-specific rows are recovered in the background only.
  // Never make the first fixture response wait again after the global budget.
  if(!merged.some(f=>f.leagueCode==="BEL")){
    Promise.resolve().then(()=>structuredLeagueFixturesForDate("BEL",centerDate))
      .then(v=>{if(v?.length)setCache(`direct-fixtures-late:${centerDate}`,mergeFixtures(lateDirect,v),300000)})
      .catch(()=>{});
  }
  if(!merged.some(f=>f.leagueCode==="RUSC")){
    Promise.resolve().then(()=>structuredLeagueFixturesForDate("RUSC",centerDate))
      .then(v=>{if(v?.length)setCache(`direct-fixtures-late:${centerDate}`,mergeFixtures(lateDirect,v),300000)})
      .catch(()=>{});
  }
  ...
  if(!merged.some(isRomanianFixture)){
    Promise.resolve().then(()=>romanianStructuredFixturesForDate(centerDate))
      .then(v=>{if(v?.length)setCache(`public-fixtures-late:${centerDate}`,mergeFixtures(latePublic,v),300000)})
      .catch(()=>{});
  }
...
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.19.3-fixture-parallel-nonblocking",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:false,timezoneNormalization:true,fixtureMerge:true,romaniaStructuredOnly:true,analysisToggle:true,marketAwareAnalysis:true,noVigMarketProbability:true}}));
...
app.get("/api/fixture-health",async(req,res)=>{
  const started=Date.now();
  try{
    const date=String(req.query.date||localYmdFromDate(new Date()));
    const result=await Promise.race([
      fixturesForThreeDays(date).then(v=>({ok:true,v})),
      new Promise(resolve=>setTimeout(()=>resolve({ok:false,timeout:true}),20000))
    ]);
    if(!result.ok)return res.status(504).json({ok:false,date,error:"fixture-timeout",elapsedMs:Date.now()-started});
    res.json({ok:true,date,count:result.v.fixtures?.length||0,elapsedMs:Date.now()-started,
      leagues:[...new Set((result.v.fixtures||[]).map(f=>f.leagueCode))]});
  }catch(e){res.status(500).json({ok:false,error:e.message,elapsedMs:Date.now()-started})}
});
...
app.get("/api/three-days",async(req,res)=>{
  try{
    ...
    const result=await fixturesForThreeDays(date);
    const days={
      [result.dates.today||date]:result.fixtures.filter(f=>f.localDate===(result.dates.today||date))
    };
    res.json({ok:true,center:date,dates:{today:result.dates.today||date},count:result.fixtures.length,days,fixtures:result.fixtures});
  }catch(e){res.status(500).json({ok:false,error:e.message});}
});
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.19.3"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.3","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function loadThreeDays(){
  const seq=++loadSeq;closeOpenAnalysis();mc.textContent="—";fc.textContent="";
  let secs=0;
  fixtures.innerHTML='<div class="loader">'+t("fixturesLoading")+' <span id="loadSecs">(0'+(lang==="tr"?"sn":"s")+')</span></div>';
  const secTimer=setInterval(()=>{
    secs++;
    const el=document.getElementById("loadSecs");
    if(el)el.textContent="("+secs+(lang==="tr"?"sn":"s")+(secs>8?(lang==="tr"?" · sunucu uyandırılıyor olabilir, lütfen bekle":" · server may be waking up, please wait"):"")+")";
  },1000);
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),18000);
  try{
    const r=await fetch("/api/three-days?date="+encodeURIComponent(SERVER_TODAY),{cache:"no-store",signal:ctrl.signal});
    const text=await r.text();
    let j;try{j=JSON.parse(text)}catch{throw Error("Sunucu JSON yerine "+r.status+" yanıtı verdi");}
    if(!r.ok||!j.ok)throw Error(j.error||("HTTP "+r.status)||t("fixtureFail"));
    if(seq!==loadSeq)return;
    allFixtures=j.fixtures||[];renderDays();chips();render();
  }catch(e){
    if(seq!==loadSeq)return;
    allFixtures=[];mc.textContent="—";
    const msg=e.name==="AbortError"?(lang==="tr"?"İstek zaman aşımına uğradı (sunucu çok yavaş yanıt verdi)":"Request timed out (server responded too slowly)"):e.message;
    fixtures.innerHTML='<div class="error">'+t("fixtureFail")+': '+esc(msg)+'<br><br><button class="analyze" onclick="loadThreeDays()">'+t("retry")+'</button></div>';
  }finally{
    clearTimeout(timer);clearInterval(secTimer);
  }
}
...
function strongestSignalCard(m,fx,id){
  ...
  const valueHtml=br.decision==="BET"
    ? '<div class="market"><b>'+t("betDecision")+' · '+esc(translateMarketName(br.name))+'</b><span class="prob">BET</span></div>'+
      ...
      '</div>'+(br.bookmaker?'<small>'+esc(br.bookmaker)+'</small>':'')+kellyHtml
