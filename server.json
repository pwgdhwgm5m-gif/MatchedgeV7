...
async function loadThreeDays(){
  ...
  try{
    const r=await fetch("/api/three-days?date="+encodeURIComponent(SERVER_TODAY),{cache:"no-store",signal:ctrl.signal}),j=await r.json();
    if(!j.ok)throw Error(j.error||t("fixtureFail"));
